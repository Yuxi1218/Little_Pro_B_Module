 ...
 is_calibrated_(false)
{
	...
	if (use_accelerometer_ || use_gyroscope_) {
		...
		PibotIMU::fillRowMajor(linear_acc_covar_, linear_acc_stdev_);

		// 角速度协方差
		pnh_.param<double>("imu/angular_vel_stdev", angular_vel_stdev_, 0.0);
		PibotIMU::fillRowMajor(angular_vel_covar_, angular_vel_stdev_);
	}
	...
	if (use_magnetometer_) {
		...
		if (use_mag_msg_) {
			...
			PibotIMU::fillRowMajor(magnetic_field_covar_, magnetic_field_stdev_);
		} else {
			// 未使能磁力计 则使用Vector3Stamped类型的topic
...
void PibotIMU::rawCallback(const pibot_msgs::RawImuConstPtr& raw_msg)
{
	...
	if (perform_calibration_ || !is_calibrated_) {
		...
		if (taken_samples < calibration_samples_) {
			...
			//acceleration_bias_["z"] = acceleration_bias_["z"] / calibration_samples_ + GRAVITY;
        	        acceleration_bias_["z"] = acceleration_bias_["z"] / calibration_samples_ + GRAVITYg;
		...
		if (use_accelerometer_ || use_gyroscope_) {
			...
			if(abs(imu_msg->angular_velocity.x)<0.01) imu_msg->angular_velocity.x =0;
			if(abs(imu_msg->angular_velocity.y)<0.01) imu_msg->angular_velocity.y =0;
			if(abs(imu_msg->angular_velocity.z)<0.01) imu_msg->angular_velocity.z =0;
			...
			imu_msg->linear_acceleration.x = (raw_msg->raw_linear_acceleration.x- acceleration_bias_["x"])*9.81;
			imu_msg->linear_acceleration.y = (raw_msg->raw_linear_acceleration.y - acceleration_bias_["y"])*9.81;
			imu_msg->linear_acceleration.z = (raw_msg->raw_linear_acceleration.z - acceleration_bias_["z"])*9.81;
...
bool PibotIMU::calibrateCallback(std_srvs::Empty::Request& request, std_srvs::Empty::Response& response)
{
...
void PibotIMU::fillRowMajor(boost::array<double, 9> & covar, double stdev)
{
